 ...
 */
export function createMatrix(row, col, value) {
  var arr = Array(row);
  for (var i = 0; i < row; i++) {
...
export function copyMatrix(row, col, matrix) {
  const matrix_ = createMatrix(row, col, 0);
  for (var x = 0; x < row; x++) {
    for (var y = 0; y < col; y++) {
...
export function checkWin(matrix) {
  ...
  for (var x = 0; x < 15; x++) {
    for (var y = 0; y < 15; y++) {
      result = check0(matrix, x, y);
      if (result.win) return result;
      result = check90(matrix, x, y);
      if (result.win) return result;
      result = check135(matrix, x, y);
      if (result.win) return result;
      result = check45(matrix, x, y);
      if (result.win) return result;
    }
  }
  return {win: false, value: 0};
